...
function InvoiceDetail({ invoice, currency }: { invoice: Invoice; currency: string }) {
  return (
...
export default function Invoices() {
  const { invoices, addInvoice, currency } = useApp();
  const [open, setOpen] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [customerName, setCustomerName] = useState("");
  const [taxRate, setTaxRate] = useState("0");
  const [lineItems, setLineItems] = useState([{ name: "", price: "", quantity: "1" }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await addInvoice({ customerName: customerName.trim(), lineItems: items, taxRate: parseFloat(taxRate) || 0 });
      setCustomerName("");
      setTaxRate("0");
                ...
                <TableBody>
                  {invoices.slice().reverse().map((inv) => (
                    <Dialog key={inv.id}>
                      ...
                      <InvoiceDetail invoice={inv} currency={currency} />
                    </Dialog>
                  ))}
                </TableBody>
              </Table>
